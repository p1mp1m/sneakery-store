...
public class ProductImageService {
    ...
    // ==========================================================
    // [8] LẤY TẤT CẢ ẢNH GALLERY THEO PRODUCT (PUBLIC API)
    // ==========================================================
    /**
     * Lấy toàn bộ ảnh gallery nhóm theo product ID
     * API public cho FE hiển thị gallery khi hover
     * 
     * @return List<ProductImagePublicDto> - danh sách ảnh theo product
     */
    @Transactional(readOnly = true)
    public List<com.sneakery.store.dto.ProductImagePublicDto> getAllImagesPublic() {
        log.info("🖼️ Fetching all product images for public gallery");
        
        // Lấy tất cả ảnh, sắp xếp theo productId và displayOrder
        List<ProductImage> allImages = productImageRepository.findAll();
        
        // Nhóm ảnh theo productId
        Map<Long, List<String>> imagesByProduct = allImages.stream()
        ...
        // Chuyển đổi sang DTO
        return imagesByProduct.entrySet().stream()
                .map(entry -> {
                    com.sneakery.store.dto.ProductImagePublicDto dto = new com.sneakery.store.dto.ProductImagePublicDto();
                    dto.setProductId(entry.getKey());
                    dto.setImages(entry.getValue());
                    return dto;
                })
