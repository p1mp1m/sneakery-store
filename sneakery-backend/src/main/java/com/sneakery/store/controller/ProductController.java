...
public class ProductController {
    ...
    /**
     * Lấy danh sách danh mục theo nhóm (parent-children structure)
     */
    @Operation(summary = "Lấy danh sách danh mục nhóm", description = "Lấy danh sách danh mục cha kèm các danh mục con. Endpoint công khai.")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Lấy danh sách thành công")
    })
    @GetMapping("/categories/groups")
    public ResponseEntity<List<CategoryDto>> getCategoryGroups() {
        List<CategoryDto> allCategories = categoryService.getAllCategories();
        
        // Lấy danh sách parent categories (parentId == null)
        List<CategoryDto> parentCategories = allCategories.stream()
                .filter(cat -> cat.getParentId() == null)
                .collect(Collectors.toList());
        
        // Với mỗi parent, tìm children và set vào
