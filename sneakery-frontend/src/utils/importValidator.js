...
export function validateProductRow(row, rowIndex, options = {}) {
  const errors = [];
  const { brands = [], categories = [], sizes = [], colors = [] } = options;
  ...
  if (!row.size || !row.size.trim()) {
    ...
    // Validate against provided sizes from API
    const validSizeNames = sizes.map((s) => String(s.name || s));
    if (!validSizeNames.includes(row.size.toString())) {
      errors.push(
        `Size "${row.size}" không hợp lệ. Size hợp lệ: ${validSizeNames.join(
          ", "
        )}`
      );
    }
  ...
  if (!row.color || !row.color.trim()) {
    ...
    // Validate against provided colors from API
    const validColorNames = colors.map((c) => (c.name || c).toLowerCase());
    if (!validColorNames.includes(row.color.toLowerCase())) {
      errors.push(
        `Màu "${row.color}" không hợp lệ. Màu hợp lệ: ${colors
          .map((c) => c.name || c)
          .join(", ")}`
      );
    }
...
export function validateImportData(rows, options = {}) {
  ...
  return {
    ...
    summary: {
      ...
      validPercentage:
        rows.length > 0
          ? Math.round((validRows.length / rows.length) * 100)
          : 0,
    },
  };
}
...
export function mapImportRow(row) {
  return {
    ...
    productName:
      row["Tên sản phẩm"] || row["Product Name"] || row["productName"] || "",
    ...
    priceBase: Number(
      row["Giá gốc"] || row["Base Price"] || row["priceBase"] || 0
    ),
    priceSale:
      row["Giá sale"] || row["Sale Price"] || row["priceSale"]
        ? Number(row["Giá sale"] || row["Sale Price"] || row["priceSale"])
        : null,
    stockQuantity: Number(
      row["Tồn kho"] || row["Stock"] || row["stockQuantity"] || 0
    ),
