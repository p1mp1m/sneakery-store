...
if (!OPENROUTER_API_KEY) {
    logger.warn('ChatbotService - OpenRouter API key not configured. Chatbot will use fallback responses.');
}
...
class ChatbotService {
    ...
    async sendMessage(messages) {
        ...
        // Nếu không có API key, dùng fallback response
        if (!OPENROUTER_API_KEY) {
            logger.warn('ChatbotService - No API key, using fallback');
            const lastUserMessage = messages[messages.length - 1]?.content || '';
            return this.getFallbackResponse(lastUserMessage);
        }
        ...
        try {
            ...
            // Xử lý lỗi thân thiện - dùng fallback response thay vì throw error
            const lastUserMessage = messages[messages.length - 1]?.content || '';
            
            if (error.response?.status === 401) {
                logger.warn('ChatbotService - API key invalid, using fallback');
                return this.getFallbackResponse(lastUserMessage);
            } else if (error.response?.status === 429) {
                return 'Xin lỗi, hiện tại tôi đang bận. Vui lòng thử lại sau ít phút hoặc liên hệ hotline để được hỗ trợ trực tiếp!';
