...
const handleReturnStatusUpdate = async () => {
  ...
  try {
    ...
    toastService.success('Thành công', `Đã cập nhật trạng thái yêu cầu trả hàng #${returnId} từ '${getReturnStatusText(previousStatus)}' sang '${getReturnStatusText(newReturnStatus.value)}' thành công!`, { duration: 3000 })
    
    showStatusConfirm.value = false
  } catch (error) {
    logger.error('❌ Lỗi khi cập nhật trạng thái:', error)
    toastService.apiError(error, 'Không thể cập nhật trạng thái yêu cầu trả hàng')
  } finally {
