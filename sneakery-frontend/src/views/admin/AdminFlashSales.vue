<template>
  <div class="max-w-[1600px] mx-auto w-full p-4 space-y-4">
      ...
      >
        <div>
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Quản lý chương trình Flash Sale giảm giá sốc
          </p>
        </div>
    ...
    <!-- ===== EMPTY STATE ===== -->
    <div
      v-else-if="filteredFlashSales.length === 0"
      class="flex flex-col items-center justify-center p-12 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700"
    >
      <div
      ...
      <p class="text-sm text-gray-500 dark:text-gray-400 text-center mb-4">
        Tạo chương trình Flash Sale đầu tiên để thu hút khách hàng
      </p>
      <button
        class="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-lg hover:from-purple-600 hover:to-purple-700 transition-all duration-200 text-sm font-medium shadow-sm"
        @click="openCreateModal"
        ...
        <table class="w-full">
          <thead class="bg-gray-50 dark:bg-gray-900/50">
            <tr>
              ...
              <th
                class="px-4 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-300 uppercase tracking-wider w-28"
              >
                Giảm giá
              </th>
              <th
              ...
              <th
                class="px-4 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-300 uppercase tracking-wider w-36"
              >
                Trạng thái
              </th>
              <th
          ...
          >
            <tr
              v-for="sale in paginatedFlashSales"
              :key="sale.id"
            ...
            >
              ...
              <td class="px-4 py-4">
                <div class="flex items-center gap-3">
                  <img
                    :src="
                      sale.productImage || productMainImageMap[sale.productId]
                    "
              ...
              <td class="px-4 py-4 whitespace-nowrap">
                <div
                  class="text-sm text-gray-600 dark:text-gray-400 line-through"
                >
              ...
              <td class="px-4 py-4 whitespace-nowrap">
                <span
                  class="inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400"
                >
              ...
              <td class="px-4 py-4 whitespace-nowrap">
                ...
                >
                  {{ formatCurrency(sale.flashSalePrice) }}
                </div>
              </td>
              <td class="px-4 py-4 whitespace-nowrap">
                ...
                <span
                  ...
                  :class="{
                    'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400':
                      getStatusClass(sale) === 'status-active',
                    'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400':
                      getStatusClass(sale) === 'status-upcoming',
                    'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400':
                      getStatusClass(sale) === 'status-expired',
                  }"
                >
    ...
    <div
      v-if="totalPages > 1"
      class="flex items-center justify-center gap-4 p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700"
    >
      <button
      ...
      >
        ...
        <div class="p-4">
          <form @submit.prevent="saveFlashSale" class="space-y-4">
            ...
            <div
              v-if="formData.productId && formData.discountPercentage"
              class="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-700 space-y-2"
            >
              ...
              <div class="flex items-center justify-between text-sm">
                <span class="text-gray-600 dark:text-gray-400"
                  >Giảm {{ formData.discountPercentage }}%:</span
                >
                <strong class="text-red-600 dark:text-red-400"
                  >-{{ formatCurrency(discountAmount) }}</strong
                >
              </div>
              <div
                class="flex items-center justify-between text-sm pt-2 border-t border-gray-200 dark:border-gray-700"
              >
                ...
                <strong
                  ...
                  >{{ formatCurrency(flashSalePrice) }}</strong
                >
              </div>
            </div>
      ...
      >
        ...
        <div class="p-4 space-y-4">
          ...
          >
            ...
            <button
              class="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-red-500 to-red-600 rounded-lg hover:from-red-600 hover:to-red-700 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            >
              <i
                ...
                >{{ deleting ? "hourglass_empty" : "delete" }}</i
              >
...
const productImageStore = useAdminProductImageStore();
...
const selectedProduct = computed(() => {
  ...
  return product && typeof product.price === "number" ? product : null;
});

const discountAmount = computed(() => {
  if (!selectedProduct.value || !formData.value.discountPercentage) return 0;
  return (
    (selectedProduct.value.price * formData.value.discountPercentage) / 100
  );
});
...
const loadFlashSales = async () => {
  try {
    ...
    const productsResult = await adminStore.fetchProducts(0, 100, {
      isActive: true,
    });
    const allProducts = (productsResult.content || []).map((p) => ({
      ...p,
      // Chuẩn hóa giá để FE dùng thống nhất
    ...
    flashSales.value = rawFlashSales.map((sale) => {
      ...
      const discountPercent = sale.discountPercent
        ? Number(sale.discountPercent)
        : 0;
      const discountAmount = (originalPrice * discountPercent) / 100;
      const flashSalePrice =
        originalPrice > 0 ? originalPrice - discountAmount : 0;
      ...
      return {
        ...
        productImage: sale.imageUrl || null,
...
const productMainImageMap = computed(() => {
  const map = {};

  Object.keys(productImageStore.imagesByProduct).forEach((productId) => {
    const images = productImageStore.imagesByProduct[productId];

    if (!images || images.length === 0) {
      map[productId] = "/placeholder-image.png";
      return;
    }

    const primary = images.find((img) => img.isPrimary === true);
    if (primary?.imageUrl) {
      map[productId] = primary.imageUrl;
      return;
    }
    ...
    map[productId] = sorted[0]?.imageUrl || "/placeholder-image.png";
  });
...
const getProductMainImage = (productId) => {
  const images = productImageStore.getImages(productId);
  ...
  // Fallback theo displayOrder
  const sorted = [...images].sort(
    (a, b) => (a.displayOrder ?? 0) - (b.displayOrder ?? 0)
  );

  return sorted[0]?.imageUrl || "/placeholder-image.png";
};
...
onMounted(async () => {
  ...
  await productImageStore.loadAllAdminImages(); // ✅ load ảnh trước
  await loadFlashSales(); // ✅ rồi mới map sale
